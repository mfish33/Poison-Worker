...
export class SimpleFileHeader implements SerializableObject {
    ...
    static read(buffer: Buffer) {
        ...
        assert.equal(
            headerMagicNumber,
            SimpleFileHeader.CORRECT_MAGIC_NUMBER,
            "Received Invalid Header Magic Number",
        );
        const version = buffer.readUInt32LE(8);
        assert.equal(
            version,
            SIMPLE_DISK_CACHE_VERSION,
            "Opened disk cache file with unexpected version",
        );
        const keyLength = buffer.readUInt32LE(12);
        const keyHash = buffer.readUInt32LE(16);
        const key = buffer
            .slice(
                SimpleFileHeader.SimpleFileHeaderSize,
                SimpleFileHeader.SimpleFileHeaderSize + keyLength,
            )
            .toString();
    ...
    serialize(): Buffer {
        const headerBuffer = Buffer.alloc(
            SimpleFileHeader.SimpleFileHeaderSize + this.keyLength,
            0,
        );
...
export function createSimpleFileFooter<T extends SerializableObject>(
    body: DeSerializableObject<T>,
) {
    ...
    class SimpleFileFooter implements SerializableObject {
        ...
        static new(body: T, hasSha256: false | string): SimpleFileFooter {
            ...
            const sha256 = hasSha256
                ? Buffer.from(crypto.createHash("sha256").update(hasSha256).digest())
                : null;
        ...
        static read(buffer: Buffer) {
            ...
            while (bufferIterator < buffer.length - 8) {
                if (
                    buffer.readBigUInt64LE(bufferIterator) === SimpleFileFooter.CORRECT_MAGIC_NUMBER
                ) {
                    break;
                }
                bufferIterator += 1;
            ...
            if (bufferIterator === buffer.length - 8) {
                throw new Error(
                    "Attempted to read body for simple file footer but ran out of the buffer",
                );
            }
            ...
            if (bodyCrc32) {
                ...
                assert.equal(
                    calculatedCrc32,
                    bodyCrc32,
                    `Crc32 of body does not match expected. Expected: ${bodyCrc32}. Got: ${calculatedCrc32}`,
                );
            }
            ...
            const footer = new SimpleFileFooter(
                footerMagicNumber,
                sha256,
                bodyCrc32,
                bodySize,
            ...
            return {
                leftover: buffer.slice(
                    SimpleFileFooter.SimpleFileFooterSize + bodySize + (sha256?.length ?? 0),
                ),
                result: footer,
            };
        }
        ...
        serialize(): Buffer {
            ...
            const footerFlags =
                (this.bodyCrc32 ? SimpleFileFooter.FLAG_HAS_CRC32 : 0) +
                (this.sha256 ? SimpleFileFooter.FLAG_HAS_KEY_SHA256 : 0);
